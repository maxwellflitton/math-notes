---
tags:
  - moc
area: misc
---
# Foundations

The prerequisite layer — notation and algebra that the rest of the vault assumes. Part of [[Home]].

## Notes
- [[sets]] — set-builder notation, membership symbols, how to read `{ x | condition }`.
- [[exp-algebria-rules]] — index laws, $e^x$, and the log laws that are their mirror image.

## Where these get used
- The set-builder definition of a line in [[sets]] is the same one that opens [[Lines]].
- The index laws in [[exp-algebria-rules]] are what let you collapse $e^{\alpha x}e^{i\beta x}$ in [[Complex conjugate roots]], exponentiate $\ln|p| = rt + C$ in [[First order differential equations]], and combine logs in [[population-example]].
- $\frac{d}{dx}e^x = e^x$ is why the exponential is the trial solution in [[linear homogenious equations]].
- Integration reference tables that lean on these: [[exponents]], [[trig]].

## Related
- [[Trigonometry]] — the other "toolkit" area.
- [[Calculus]] — the first area that consumes these rules in bulk.
